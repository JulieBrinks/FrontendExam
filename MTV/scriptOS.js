...
function sendBesked() {
    ...
    let gt = new Date();
    firebase.database().ref("OldSchool").push().set({
        "Afsender": userName,
        "ChatBesked": Besked,
        "tidT": gt.getHours(),
        "tidM": gt.getMinutes()
...
//fange nye beskeder
firebase.database().ref("OldSchool").on("child_added", function(snapshot){
    let OS = "";
    OS += "<li>";
    if(snapshot.val().tidM < 10) {
        OS += snapshot.val().Afsender + ": " + snapshot.val().ChatBesked + " kl: " + snapshot.val().tidT+":"+"0"+snapshot.val().tidM;
    } else if(snapshot.val().tidT < 10) {
        OS += snapshot.val().Afsender + ": " + snapshot.val().ChatBesked + " kl: " +"0"+ snapshot.val().tidT+":"+snapshot.val().tidM;
    }
    OS += "</li>";
